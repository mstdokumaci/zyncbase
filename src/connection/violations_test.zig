...
test "ConnectionViolationTracker: basic functionality" {
    ...
    // First violation
    {
        ...
        try testing.expectEqual(@as(u32, 1), test_helpers.getViolationCount(&tracker, conn_id));
    }
    ...
    // Second violation
    {
        const should_close = try tracker.recordViolation(conn_id);
        try testing.expect(!should_close);
        try testing.expectEqual(@as(u32, 2), test_helpers.getViolationCount(&tracker, conn_id));
    }
    ...
    // Third violation - should trigger closure
    {
        const should_close = try tracker.recordViolation(conn_id);
        try testing.expect(should_close);
        try testing.expectEqual(@as(u32, 3), test_helpers.getViolationCount(&tracker, conn_id));
    }

    tracker.clearViolations(conn_id);
    try testing.expectEqual(@as(u32, 0), test_helpers.getViolationCount(&tracker, conn_id));
}
...
test "ConnectionViolationTracker: multiple connections" {
    ...
    try testing.expectEqual(@as(u32, 1), test_helpers.getViolationCount(&tracker, conn1));
    try testing.expectEqual(@as(u32, 0), test_helpers.getViolationCount(&tracker, conn2));
    ...
    try testing.expect(should_close);
    try testing.expectEqual(@as(u32, 1), test_helpers.getViolationCount(&tracker, conn1));
    try testing.expectEqual(@as(u32, 2), test_helpers.getViolationCount(&tracker, conn2));
}
