...
pub const SessionResolver = struct {
    ...
    fn deliverResult(self: *SessionResolver, result: SessionResolutionResult, cm: *ConnectionManager) void {
        ...
        if (result.err) |err| {
            if (!conn.isScopeSeqCurrentFor(result.scope_seq, result.is_presence)) {
        ...
        const pending_namespace = conn.dupePendingNamespaceIfSeq(arena.allocator(), result.scope_seq, result.is_presence) catch |err| {
            self.sendError(conn, result.msg_id, err);
            return;
        } orelse {
            self.sendStaleScopeError(conn, result.msg_id);
        ...
        if (!conn.setScopeIfSeq(result.scope_seq, result.namespace_id, result.user_doc_id, result.is_presence)) {
            self.sendStaleScopeError(conn, result.msg_id);
            return;
        }
