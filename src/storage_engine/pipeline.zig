...
pub fn executeInsert(
    ...
    if (sqlite.c.sqlite3_bind_text(stmt.stmt, bind_idx, id_z.ptr, @intCast(op.id.len), types.sqlite_transient) != sqlite.c.SQLITE_OK) return reader.classifyStepError(conn);
    bind_idx += 1;
    if (sqlite.c.sqlite3_bind_text(stmt.stmt, bind_idx, ns_z.ptr, @intCast(op.namespace.len), types.sqlite_transient) != sqlite.c.SQLITE_OK) return reader.classifyStepError(conn);
    bind_idx += 1;
...
pub fn executeUpdate(
    ...
    if (sqlite.c.sqlite3_bind_text(stmt.stmt, 1, id_z.ptr, @intCast(op.id.len), types.sqlite_transient) != sqlite.c.SQLITE_OK) return reader.classifyStepError(conn);
    if (sqlite.c.sqlite3_bind_text(stmt.stmt, 2, ns_z.ptr, @intCast(op.namespace.len), types.sqlite_transient) != sqlite.c.SQLITE_OK) return reader.classifyStepError(conn);
