...
/// Values-only variant: renders just the bind values for a guard predicate,
/// without building the SQL fragment. Used on the cache-hit path.
/// Returns null when the predicate is always-true (no values needed).
pub fn renderGuardValues(
    allocator: Allocator,
    table_metadata: *const schema_types.Table,
    predicate: ?*const query_ast.FilterPredicate,
) !?[]Value {
    ...
    try appendFilterValues(allocator, &values, table_metadata, pred);

    return try values.toOwnedSlice(allocator);
}

/// SQL-only variant: renders just the SQL fragment for a guard predicate,
/// without building values. Used on the cache-miss path.
/// Returns null when the predicate is always-true (no fragment needed).
pub fn renderGuardSql(
    allocator: Allocator,
    table_metadata: *const schema_types.Table,
    predicate: ?*const query_ast.FilterPredicate,
) !?[]const u8 {
    ...
    try buf.appendSlice(allocator, " AND (");
    try appendFilterPredicateSqlPlaceholders(allocator, &buf, table_metadata, pred);
    try buf.append(allocator, ')');
...
/// Values-only variant of appendFilterPredicateSql — walks the predicate and
/// builds the bind values array without emitting any SQL. Used on the cache-hit
/// path where the SQL template is already cached.
pub fn appendFilterValues(
    allocator: Allocator,
    values: *std.ArrayListUnmanaged(Value),
    table_metadata: *const schema_types.Table,
    predicate: *const query_ast.FilterPredicate,
    ...
    _ = table_metadata;
    if (predicate.isAlwaysTrue() or predicate.isAlwaysFalse()) return;

    const conds = predicate.conditions orelse @as([]const query_ast.Condition, &.{});
    for (conds) |*cond| {
        try appendConditionValues(allocator, values, cond);
    }
...
/// SQL-only variant of appendFilterPredicateSql — emits `?` placeholders
/// without touching a values array. Used on the cache-miss path to build
/// the SQL template for preparing a new statement.
pub fn appendFilterPredicateSqlPlaceholders(
    allocator: Allocator,
    buf: *SqlBuf,
    table_metadata: *const schema_types.Table,
    predicate: *const query_ast.FilterPredicate,
    ...
    var and_list = SqlList.init(buf, " AND ");
    for (conds) |*cond| {
        try appendConditionSqlPlaceholders(allocator, &and_list, table_metadata, cond);
    }
    ...
    for (clauses) |clause| {
        ...
        var or_list = SqlList.init(buf, " OR ");
        for (clause) |*cond| {
            try appendConditionSqlPlaceholders(allocator, &or_list, table_metadata, cond);
        }
        try buf.append(allocator, ')');
    }
}
...
// ─── Split variants for cache-hit (values-only) and cache-miss (SQL-only) paths ───

/// Extracts bind values from a single condition without emitting SQL.
fn appendConditionValues(
    allocator: Allocator,
    values: *std.ArrayListUnmanaged(Value),
    cond: *const query_ast.Condition,
) !void {
    switch (cond.op) {
        ...
        .contains => {
            ...
            if (cond.field_type == .array) {
                ...
                if (val != .scalar or val.scalar != .text) return error.InvalidConditionValue;
                const escaped = try escapeLikePattern(allocator, val.scalar.text);
                try appendOwnedValue(allocator, values, Value{ .scalar = .{ .text = escaped } });
            }
        },
...
/// Emits SQL with `?` placeholders for a single condition without touching values.
fn appendConditionSqlPlaceholders(
    allocator: Allocator,
    list: *SqlList,
    table_metadata: *const schema_types.Table,
    cond: *const query_ast.Condition,
