...
test "WriteWorker: flushBatch throughput" {
    ...
    const target_flush_10: f64 = if (is_tsan) 36.0 else if (is_debug) 9.0 else 3.0;
    const target_drain_10: f64 = if (is_tsan) 4.8 else if (is_debug) 1.2 else 0.4;
    const target_flush_100: f64 = if (is_tsan) 300.0 else if (is_debug) 75.0 else 25.0;
    const target_drain_100: f64 = if (is_tsan) 42.0 else if (is_debug) 10.5 else 3.5;
