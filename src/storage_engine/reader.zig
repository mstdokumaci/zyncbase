...
pub fn bindTypedValue(stmt: sqlite.DynamicStatement, index: c_int, value: TypedValue) !void {
    const rc = switch (value) {
        ...
        .text => |s| sqlite.c.sqlite3_bind_text(stmt.stmt, index, s.ptr, @intCast(s.len), types.sqlite_transient),
        .boolean => |b| sqlite.c.sqlite3_bind_int(stmt.stmt, index, if (b) 1 else 0),
        .blob => |b| sqlite.c.sqlite3_bind_blob(stmt.stmt, index, b.ptr, @intCast(b.len), types.sqlite_transient),
...
pub fn execSelectDocument(
    ...
    if (sqlite.c.sqlite3_bind_text(stmt.stmt, 1, id_z.ptr, @intCast(id.len), types.sqlite_transient) != sqlite.c.SQLITE_OK) return classifyStepError(reader);
    if (sqlite.c.sqlite3_bind_text(stmt.stmt, 2, ns_z.ptr, @intCast(namespace.len), types.sqlite_transient) != sqlite.c.SQLITE_OK) return classifyStepError(reader);
...
pub fn execSelectCollection(
    ...
    if (sqlite.c.sqlite3_bind_text(stmt.stmt, 1, ns_z.ptr, @intCast(namespace.len), types.sqlite_transient) != sqlite.c.SQLITE_OK) return classifyStepError(reader);
