...
pub fn lockFreeCache(comptime t: type, comptime KeyType: type) type { // zwanzig-disable-line: unused-parameter
    ...
    return struct {
        ...
        pub fn init(self: *Self, io: std.Io, allocator: Allocator, config: Config) !void {
        ...
        pub fn deinit(self: *Self) void {
            ...
            self.reclaim_event.set(self.io);
        ...
        fn reclaimLoop(self: *Self) void {
            while (self.reclaim_active.load(.acquire)) {
                self.reclaim(false);
                self.reclaim_event.reset();
                if (!self.reclaim_active.load(.acquire)) break;
                _ = self.reclaim_event.waitTimeout(self.io, .{ .duration = .{
                    .raw = std.Io.Duration.fromMilliseconds(@intCast(self.config.reclamation_interval_ms)),
                    .clock = .awake,
