...
test "StorageEngine: match-none guard permits insert branch and blocks update branch" {
    ...
    try ctx.engine.upsertDocument(table.index, 1, 1, 1, &insert_columns, &guard, null, null);
    try tasks.flush();
    ...
    try ctx.engine.upsertDocument(table.index, 1, 1, 1, &update_columns, &guard, null, null);
    try tasks.flush();
