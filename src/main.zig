...
pub const ZyncBaseServer = struct {
    ...
    pub fn initDetailed(allocator: std.mem.Allocator, custom_config: ?Config, custom_data_dir: ?[]const u8) !*ZyncBaseServer {
        ...
        errdefer {
            // Only deinit if we owned it (not passed in)
            // Wait, actually initDetailed should take ownership of the config to simplify cleanup
            config.deinit();
        }
...
pub fn main() !void {
    ...
    // Initialize server
    const server = ZyncBaseServer.init(allocator) catch |err| {
