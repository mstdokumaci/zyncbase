...
test "ThreadBudget with 8 cores" {
    ...
    try std.testing.expectEqual(@as(usize, 2), budget.subscription);
...
test "ThreadBudget with 16 cores" {
    ...
    try std.testing.expectEqual(@as(usize, 8), budget.subscription);
...
test "ThreadBudget with 32 cores" {
    ...
    try std.testing.expectEqual(@as(usize, 4), budget.readers);
    try std.testing.expectEqual(@as(usize, 24), budget.subscription);
