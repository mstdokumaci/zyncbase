 ...
 *
 * Bun's fork of uWebSockets includes calls to Bun-specific functions that don't
 * exist in the standard uWebSockets library. These stubs allow us to link
 * successfully while providing minimal implementations.
 */
...
void __attribute__((__noreturn__)) Bun__panic(const char *message,
                                              size_t length) {
  (void)message;
  (void)length;
  abort();
...
void set_bun_is_exiting(int exiting) { is_exiting_flag = exiting; }
...
ssize_t sys_epoll_pwait2(int epfd, struct epoll_event *events, int maxevents,
