...
test "migration_executor: 5.5 - destructive migration preserves common-column data" {
    ...
    // Insert a row
    try execSql(&db, allocator, "INSERT INTO tasks (id, namespace_id, owner_id, title, status, created_at, updated_at) VALUES (zeroblob(16), 1, 'test-owner', 'My Task', 'open', 0, 0)");
...
test "migration_executor: 5.6 - mid-migration failure leaves database unchanged" {
    ...
    // Insert a row
    try execSql(&db, allocator, "INSERT INTO real_table (id, namespace_id, owner_id, name, created_at, updated_at) VALUES (zeroblob(16), 1, 'test-owner', 'test', 0, 0)");
