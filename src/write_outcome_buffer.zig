...
pub const WriteOutcomeResult = struct {
    conn_id: u64,
    write_id: [16]u8,
    err: ?anyerror,
    /// Index of the failing operation within a StoreBatch, when known.
    /// null for single-op writes or transaction-level failures.
    batch_index: ?usize = null,
};

pub const WriteOutcomeBuffer = struct {
    buffer: []WriteOutcomeResult,
    write_pos: std.atomic.Value(usize),
    read_pos: std.atomic.Value(usize),
    overflow: std.ArrayListUnmanaged(WriteOutcomeResult),
    overflow_mutex: std.Thread.Mutex,
    allocator: Allocator,

    const capacity = 256;

    pub fn init(allocator: Allocator) !WriteOutcomeBuffer {
        const buffer = try allocator.alloc(WriteOutcomeResult, capacity);
        return .{
            .buffer = buffer,
    ...
    pub fn push(self: *WriteOutcomeBuffer, result: WriteOutcomeResult) !void {
    ...
    pub fn drainInto(
        self: *WriteOutcomeBuffer,
        out: *std.ArrayListUnmanaged(WriteOutcomeResult),
        allocator: Allocator,
    ) !void {
    ...
    pub fn deinit(self: *WriteOutcomeBuffer) void {
