...
const validFieldName = fc
  ...
  .filter((s) => !s.includes("__") && !s.includes(".") && /^[a-zA-Z_][a-zA-Z0-9_]*$/.test(s));
...
const schemaField: fc.Arbitrary<SchemaField> = fc.oneof(
  { weight: 3, arbitrary: leafField },
  {
    ...
    arbitrary: fc.record({
      ...
      fields: fc.dictionary(validFieldName, leafField, { minKeys: 1, maxKeys: 4 }),
    }),
  }
);
...
const schemaCollection = fc.record({
  fields: fc.dictionary(validFieldName, schemaField, { minKeys: 1, maxKeys: 5 }),
  required: fc.option(fc.array(validFieldName, { minLength: 0, maxLength: 3 }), { nil: undefined }),
});

/** Generate a SchemaFile */
const schemaFileArb = fc.record({
  version: fc.constant("1"),
  store: fc.dictionary(validFieldName, schemaCollection, { minKeys: 1, maxKeys: 4 }),
});
  ...
  test("Property 13: ValidPaths contains a tuple for every derivable path", () => {
    fc.assert(
      fc.property(schemaFileArb, (schemaFile) => {
        ...
        for (const [collectionName, collection] of Object.entries(schemaFile.store)) {
  ...
  test("Property 12: generated types compile without errors", () => {
    fc.assert(
      fc.property(schemaFileArb, (schemaFile) => {
        ...
        const typesFile = path.join(tmpDir, `zyncbase-types-${Date.now()}-${Math.random().toString(36).slice(2)}.ts`);
        const importFile = path.join(tmpDir, `zyncbase-import-${Date.now()}-${Math.random().toString(36).slice(2)}.ts`);
