...
import type { OutboundRequest } from "./connection_wire";
import { ZyncBaseError } from "./errors";
import { type StoreConnection, StoreImpl } from "./store";
...
/** StoreConnection + the extra hooks used by ZyncBaseClient wiring in tests. */
interface MockConn extends StoreConnection {
	onDelta(handler: (delta: StoreDelta) => void): void;
}

function makeConn(
	dispatchFn: (msg: OutboundRequest) => Promise<OkResponse>,
	onDeltaFn: (handler: (delta: StoreDelta) => void) => void = () => {},
): MockConn {
	return {
