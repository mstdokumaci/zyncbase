...
import { normalizePath, encodeWirePath, decodeWirePath } from "./path";
      ...
      fc.property(
        ...
        (segments) => {
          ...
          try {
            ...
            if (e instanceof ZyncBaseError && e.code === ErrorCodes.INVALID_PATH) {
              threw = true;
            }
          }
          return threw;
...
describe("encodeWirePath / decodeWirePath", () => {
      ...
      fc.property(
        ...
        fc.array(
          fc.string({ minLength: 1 })
            .filter((s) => !s.includes("__") && !s.includes(".") && !s.startsWith("_") && !s.endsWith("_")), 
          { minLength: 3 }
        ),
        (path) => {
  ...
  test("Property 16: depth-1 paths are returned unchanged by encodeWirePath", () => {
    fc.assert(
      fc.property(
        fc.array(fc.string({ minLength: 1 }).filter((s) => !s.includes("__")), { minLength: 1, maxLength: 1 }),
        (path) => {
  ...
  test("Property 16: depth-2 paths are returned unchanged by encodeWirePath", () => {
    fc.assert(
      fc.property(
        fc.array(fc.string({ minLength: 1 }).filter((s) => !s.includes("__")), { minLength: 2, maxLength: 2 }),
        (path) => {
          expect(encodeWirePath(path)).toEqual(path);
        }
      ),
