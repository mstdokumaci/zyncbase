...
export async function run(
	mode: "set" | "get",
	port: number = 3000,
	artifactDir: string = "test-artifacts/e2e",
) {
	const client = new ZyncBaseClient(`ws://localhost:${port}`);
	const timestampFile = path_pkg.join(
		process.cwd(),
		artifactDir,
		"persistence_timestamp.txt",
	);
	...
	try {
		...
		if (mode === "set") {
			...
			const expectedTimestamp = parseInt(
				fs.readFileSync(timestampFile, "utf-8"),
				10,
			);
			...
			if (
				...
				data?.timestamp === expectedTimestamp
			) {
				console.log("Persistence verified with exact timestamp match.");
			} else {
				throw new Error(
					`Persistence verification failed. Expected timestamp ${expectedTimestamp}, got ${data?.timestamp}`,
				);
			}
		}
	} catch (err) {
