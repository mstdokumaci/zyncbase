...
Every SQLite query generated by the query engine includes a `WHERE namespace_id = ?` clause. The `namespace_id` is set from the authenticated session context, never from client-supplied data. Cross-namespace access requires an explicit authorization grant from the Hook Server.
