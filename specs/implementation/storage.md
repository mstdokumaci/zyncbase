...
const MigrationExecutor = struct {
    ...
    fn recreateTable(self: *MigrationExecutor, table: Table) !void {
        // Strategy: 
        // 1. CREATE TABLE table_backup AS SELECT * FROM table;
        // 2. DROP TABLE table;
        // 3. CREATE TABLE table (...);
        // 4. INSERT INTO table SELECT ... FROM table_backup;
        // 5. DROP TABLE table_backup;
    }
};
```
...
### Online Backup
Performs atomic backups during operation by checkpointing WAL and performing a filesystem-level copy or utilizing SQLite's backup API.

```zig
pub fn backup(self: *StorageLayer, dest_path: []const u8) !void {
    // Checkpoint WAL first to ensure main DB is up to date
    try self.write_conn.exec("PRAGMA wal_checkpoint(FULL)");
    
    // Copy database file
    try std.fs.copyFileAbsolute("ZyncBase.db", dest_path, .{});
}
```
