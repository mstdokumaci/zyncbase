...
- User-defined unique constraints compile to managed SQLite unique indexes whose first column is always `namespace_id`. Index names are deterministic: existing managed indexes use `"idx_<table>_..."`, user unique indexes use `"uidx_<table>_<ordinal>"` (zero-based, schema order). Both prefixes are reserved per table.
