...
- **Namespaced Users:** If `users` is configured with `"namespaced": true`, identity resolution uses the namespace ID of the scope being established. Store and presence scopes may resolve to different internal user IDs when their namespaces differ. In this mode, namespace switching is not a supported usage pattern: a user's identity is resolved in the context of their home namespace, and accessing a different namespace would create a new, unrelated identity row in that namespace's partition. The server enforces this by rejecting namespace switch requests with `NAMESPACE_SWITCH_REJECTED` when `users.namespaced` is enabled and a scope is already active. Applications using `users.namespaced = true` should ensure each connection operates within a single fixed namespace.
