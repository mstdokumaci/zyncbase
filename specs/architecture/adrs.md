...
- `created_at` — immutable Unix microseconds, unique within its table, and generated by a per-table logical clock on the sole writer as `max(wall_time_us, last + 1)`. Startup restores each clock from indexed `MAX(created_at)`; rollback gaps are valid. Values may repeat across tables.
- `updated_at` — server wall-clock Unix microseconds. It changes on explicit updates but is not unique or monotonic.
...
Subscribers with identical `(namespace, collection, where, orderBy)` share one evaluation group. `orderBy` participates in group identity as the parsed **canonical order**: omitted order becomes `created_at ASC`, non-unique explicit orders receive hidden `created_at ASC`, and explicit final `created_at` or `id` remains unchanged because either is already unique. Omitted order and `[{ created_at: 'asc' }]` therefore canonicalize identically and share groups. A `RecordChange` event evaluates each unique group exactly once — not once per subscriber. `limit` is per-subscriber metadata, applied after group evaluation. New subscribers joining an existing warm group receive the current snapshot immediately.
