...
STX v2.0 is built on four core principles that guide every architectural decision:
...
|----------|--------------|-------------|-------------|-----------|-------------|
| **Zig (STX)** | Manual / Explicit | Native Threads | None | Zero-cost ABI | ~15MB |
...
**Key Insight**: uWebSockets provides the same performance as Bun because they use the same underlying engine. By using it directly, STX achieves Bun-level performance.
...
STX follows a "configuration-first" approach inspired by infrastructure tools:
...
Users configure STX with three JSON files:

1. **stx.config.json** - Server settings, auth, namespaces
...
STX v2.0 targets the following performance characteristics:
...
Every architectural decision involves trade-offs. Here are STX's conscious limitations:
