...
STX's presence system tracks ephemeral user state (cursors, typing indicators, online status) in real-time with sub-100ms latency. All presence data is stored in-memory only—never persisted to SQLite.
