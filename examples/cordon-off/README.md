...
A small multiplayer territory game using ZyncBase's real actions → store path. The browser joins and sends direction input through actions; a Bun simulation determines movement and ownership. Browsers receive territory and player coordinates through two independently sized chunk-table subscriptions.
...
| `GAME_ROUND_MS` | `3600000` | Round boundary period; the default aligns with the top of each UTC hour. |
...
- Country creation sends `countryName` once to `/session`, which returns the assigned `country_id`. The sync `player_join` action admits the player with that id and a display name, and returns the internal user id that roster rows and dots are keyed by. A newly allocated country with no land or players is removed after ten seconds if its creator never joins.
- The async `player_move` action carries direction and sequence; heartbeats renew a two-second movement lease, so silent players stop and their dots expire after ten seconds. Each session also holds a slot for twice that window, which outlives the player's ten-second tombstone: a short disconnect reconnects with the same token and resumes on the tombstone cell, and a still-connected tab that slept through its lease re-admits itself on its next heartbeat. Beyond that window the session is released and the browser returns to the lobby. A best-effort `player_leave` on page hide removes the dot immediately; crashes and dropped sockets fall back to the lease. Rejoining preserves country territory, not the old player position.
...
- Players can only invoke `player_join`, `player_move`, and `player_leave`; they cannot register action workers or write country chunks, user chunks, or countries. The browser exchanges no presence data and does not subscribe to presence — the SDK still opens an empty presence namespace for identity plumbing, and a custom client could inspect it, but there is no hidden game information there.
...
- Rounds are wall-clock anchored: boundaries are absolute multiples of `GAME_ROUND_MS` (1 h → top of each UTC hour), so restarts resume the same deadline. A round is archived when it has claimed land, even if every human left before the boundary, provided the boundary arrives before an idle reset; at most 36 results are kept, and every round end or idle reset exits the process so the supervisor starts a clean simulation and ZyncBase pair. `dist/history/` is the archive and the Worker publish source; the view is `history.html`, a static page that polls for results while they publish.
